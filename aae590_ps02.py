...
def se2_exp(xi: np.ndarray) -> np.ndarray:
    ...
    # sin x / x
    sinx_over_x = (
        1 - theta**2 / 6 + theta**4 / 120 - theta**6 / 5040 + theta**8 / 362880
    )
    # (1 - cos x) / x
    ...
    if np.isclose(theta, 0):
        ...
        V = (
            np.eye(2) * np.sin(theta) / theta
            + np.array([[0, -1], [1, 0]]) * (1 - np.cos(theta)) / theta
        )
...
def se2_log(X: np.ndarray) -> np.ndarray:
    ...
    x_over_2_cot_x_over_2 = (
        1 - theta**2 / 12 - theta**4 / 720 - theta**6 / 30240 - theta**8 / 1209600
    )
    ...
    if np.isclose(theta, 0):
        ...
        V_inv = np.eye(2) * (theta / 2) * (1 / np.tan(theta / 2)) + np.array(
            [[0, 1], [-1, 0]]
        ) * (theta / 2)
...
def plot_trajectories(X_euler, X_lie_group):
    ...
    plt.plot(
        lie_group_positions[:, 0],
        lie_group_positions[:, 1],
        label="Lie Group Integration",
        marker="x",
    )
...
def plot_frobenius_error(X_euler, X_lie_group):
    ...
    for i in range(num_steps):
        ...
        lie_group_errors[i] = np.linalg.norm(
            R_lie_group.T @ R_lie_group - np.eye(2), ord="fro"
        )
...
def test_adjointX_inv_equals_adjX_inv(X):
    ...
    assert np.allclose(adj_X_inv, adj_X_inv_direct), (
        "Adjoint inverse property test failed!"
    )
...
def test_exact_integration_equals_lie_group_integration(xi, X, t, dt):
    ...
    assert np.allclose(exact_X, X_lie_group[-1], rtol=1e-1, atol=1e-1), (
        "Exact integration and Lie group integration test failed!"
    )
